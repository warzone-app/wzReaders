...
class Graph extends Component {
  ...
  componentDidMount() {
    ...
    this.createLineChart(
      `#KillsLineChartDiv`,
      this.formateData(this.mapKills(this.props.userMatch.data.matches)),
      "",
      "Kills",
      200
    );
    this.createLineChart(
      `#DmgLineChartDiv`,
      this.formateData(this.mapDmg(this.props.userMatch.data.matches)),
      "",
      "Damage",
      300,
      615,
    ...
    this.createCircleMeter(
      this.mapGulagWinsToTotal(this.props.userMatch.data.matches)
    );
  }
  ...
  mapGW(arr) {
    ...
    arr.map((el, i) => {
      if (i > 9) {
        ...
        if (
          el.playerStats.gulagKills === 0 &&
          el.playerStats.gulagDeaths === 0
        ) {
          result.push("-");
  ...
  createCircleMeter(arr) {
    ...
    const arc = d3.arc().startAngle(0).innerRadius(78).outerRadius(96);

    const svg = d3
      .select("#GWCircleChartDiv")
      .append("svg")
      .attr("width", width)
      .attr("height", height)
      .append("g")
          ...
          <div id="playerGWDisplayContainer">
            ...
            <div id="playerGWDisplay">
              {this.mapGW(this.props.userMatch.data.matches).map((el, i) => {
                if (el === 1) {
                  return (
                    <div className="GulagIndivW" key={i}>
                      W
                    </div>
                  );
                } else if (el === 0) {
                  return (
                    <div className="GulagIndivL" key={i}>
                      L
                    </div>
                  );
                } else {
                  return (
                    <div className="GulagIndiv-" key={i}>
                      {el}
                    </div>
                  );
                }
